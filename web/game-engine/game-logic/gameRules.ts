 ...
 */
export function canPlaceSettlement(intId: string, graph: any, players: any[], playerId: string): boolean {
  ...
  if (totalBuildings < 2) {
    ...
    return inter.adjacentEdges.some((eid: any) => !allRoads.has(eid));
  }

  // Main game: must connect to own road network
  const myRoads = new Set(player.roads);
  return inter.adjacentEdges.some((eid: any) => myRoads.has(eid));
}

// --- City ---

/** Can upgrade to city only on the player's own existing settlement. */
export function canUpgradeCity(intId: string, players: any[], playerId: string): boolean {
  const player = players.find((p) => p.id === playerId);
 ...
 */
export function canPlaceRoad(edgeId: string, graph: any, players: any[], playerId: string): boolean {
  ...
  for (const intId of edge.intersections) {
    ...
    if (!opponentBuildings.has(intId)) {
      const inter = graph.intersections.get(intId);
      if (inter.adjacentEdges.some((eid: any) => eid !== edgeId && myRoads.has(eid))) {
        return true;
      }
    }
  }
 ...
 */
export function computeLongestRoad(playerId: string, graph: any, players: any[]): number {
  const player = players.find((p) => p.id === playerId);
 ...
 */
export function distributeResources(diceSum: number, hexes: any[], graph: any, players: any[], robberHex: string): any[] {
  ...
  for (const hex of hexes) {
    ...
    const resource = TERRAIN_CONFIG[hex.type].resource.toLowerCase();
